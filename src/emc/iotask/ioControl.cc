...
#define UNEXPECTED_MSG fprintf(stderr,"UNEXPECTED idx %s %d\n",__FILE__,__LINE__);
...
void load_tool(int idx) {
    ...
    if(random_toolchanger) {
        ...
        tooldata_put(tzero,  idx); // spindle-->pocket
        tooldata_put(tpocket,0  ); // pocket-->spindle

        comment_temp = ttcomments[0];
        ttcomments[0] = ttcomments[idx];
        ttcomments[idx] = comment_temp;

        if (0 != SAVETABLE(io_tool_table_file, emcioStatus.tool.toolTable,
...
{
#ifdef TOOL_MMAP //{
    fprintf(stderr,"%8d IO: TOOLTABLE:mmap\n",getpid());
#else //}{
    fprintf(stderr,"%8d IO: TOOLTABLE:nml\n",getpid());
#endif //}
    ...
    if (0 != LOADTABLE(io_tool_table_file, emcioStatus.tool.toolTable,
                       ttcomments, random_toolchanger)) {
    ...
    *(iocontrol_data->tool_number) = emcioStatus.tool.toolInSpindle;
    ...
    while (!done) {
        ...
        case EMC_TOOL_INIT_TYPE:
            LOADTABLE(io_tool_table_file, emcioStatus.tool.toolTable,
                      ttcomments, random_toolchanger);
        ...
        case EMC_TOOL_PREPARE_TYPE:
            {
                ...
                if (idx == -1) {  // not found
                    ...
                    *(iocontrol_data->tool_prep_pocket) = random_toolchanger? idx: tdata.pocketno;
                    ...
                    if(!random_toolchanger && idx == 0) {//unload spindle
        ...
        case EMC_TOOL_SET_OFFSET_TYPE: 
            {
                ...
                rtapi_print_msg(RTAPI_MSG_DBG,
                                "EMC_TOOL_SET_OFFSET idx=%d toolno=%d zoffset=%lf, xoffset=%lf, diameter=%lf,"
